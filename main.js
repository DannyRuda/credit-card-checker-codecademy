...
function validateCred(array) {
    ...
    for (let i = 0; i < array.length; i++) {
        if(i !== 0 &&  i%2 !== 0) {
           if (array[array.length-1 - i]*2 > 9 ) {
               luhnSum +=  array[array.length-1 - i]*2 - 9;
           } else {
            luhnSum +=  array[array.length-1 - i]*2 ;
           };
        }
        else {
            luhnSum += array[array.length-1 - i];
        }
    }
    console.log(luhnSum);
    return luhnSum%10 === 0;
...
function findInvalidCards(nestArray) {
    return nestArray.filter(cardNumbers => !validateCred(cardNumbers));
} 

function idInvalidCardCompanies(invalidCards) {
    let invalidCompanies = [];
    for (let cardNumber of invalidCards) {
        switch (cardNumber[0]) {
            case 3: 
